...
exports.PreHookProcessor = void 0;
...
class PreHookProcessor {
    constructor(config, intentManager, stateMachine, scopeEnforcer, traceLogger, requestHITLApproval) {
        this.config = config;
        this.intentManager = intentManager;
        this.stateMachine = stateMachine;
        this.scopeEnforcer = scopeEnforcer;
        this.concurrencyGuard = ConcurrencyGuard_1.ConcurrencyGuard;
    ...
    async process(request) {
        ...
        const risk = CommandClassifier_1.CommandClassifier.classify(toolName, parameters);
        if (risk === "SAFE") {
        ...
        if (session.phase !== "INTENT_LOADED") {
            ...
            return {
                ...
                error: {
                    ...
                    recoveryHint: "You MUST call select_active_intent(intent_id) before making any " +
        ...
        if (CommandClassifier_1.CommandClassifier.isMutating(toolName)) {
            const targetPath = CommandClassifier_1.CommandClassifier.extractTargetPath(toolName, parameters);
        ...
        if (this.config.enableConcurrencyGuard && CommandClassifier_1.CommandClassifier.isMutating(toolName)) {
            const targetPath = CommandClassifier_1.CommandClassifier.extractTargetPath(toolName, parameters);
            if (targetPath) {
        ...
        if (this.config.enableHITL) {
            if (risk === "ELEVATED") {
                const cmd = parameters["command"] ||
                    CommandClassifier_1.CommandClassifier.extractTargetPath(toolName, parameters) ||
                    toolName;
                const approved = await this.requestHITLApproval(`⚠️ ELEVATED RISK ACTION\n\nIntent: ${activeIntent.id} — ${activeIntent.name}\nTool: ${toolName}\nCommand: ${cmd}\n\nApprove this action?`);
                if (!approved) {
                    return {
                        allowed: false,
                        error: {
                            ...
                            recoveryHint: "The human reviewer rejected this action. Reconsider your approach. " +
            ...
            const mutationClass = this.inferMutationClass(toolName, parameters, activeIntent);
            if (mutationClass === "INTENT_EVOLUTION" && risk === "DESTRUCTIVE") {
                const filePath = CommandClassifier_1.CommandClassifier.extractTargetPath(toolName, parameters) ?? "unknown";
                const approved = await this.requestHITLApproval(`🔄 INTENT EVOLUTION DETECTED\n\nIntent: ${activeIntent.id} — ${activeIntent.name}\nFile: ${filePath}\n\nThis write appears to EXTEND scope, not just refactor. Approve?`);
                if (!approved) {
                    return {
                        allowed: false,
                        error: {
                            ...
                            recoveryHint: "The human rejected this scope expansion. Limit your changes to " +
    ...
    // ── Handshake Handler ────────────────────────────────────
    async handleSelectIntent(request) {
        const { parameters, sessionId } = request;
        const intentId = parameters["intent_id"] ?? "";
        if (!intentId) {
            return {
                allowed: false,
                error: {
                    ...
                    recoveryHint: "Call select_active_intent with a valid intent_id string, " +
        ...
        if (!this.intentManager.isValidIntent(intentId)) {
            return {
                ...
                error: {
                    ...
                    recoveryHint: `'${intentId}' is not a valid active intent. ` +
        ...
        const recentTrace = this.traceLogger
            ...
            .slice(-5)
            .map((e) => `  [${e.timestamp}] ${e.files[0]?.relative_path ?? "?"} — ${e.files[0]?.mutation_class ?? "?"}`)
            .join("\n");
        const injectedContext = contextBlock +
            (recentTrace
    ...
    // ── Helpers ──────────────────────────────────────────────
    snapshotIfReadFile(toolName, parameters, sessionId) {
    ...
    inferMutationClass(toolName, parameters, activeIntent) {
        ...
        const hasUnrelatedExport = newExports.some((exp) => !intentKeywords.some((kw) => exp.toLowerCase().includes(kw)));
        return hasUnrelatedExport ? "INTENT_EVOLUTION" : "AST_REFACTOR";
    }
}
exports.PreHookProcessor = PreHookProcessor;
//# sourceMappingURL=PreHookProcessor.js.map