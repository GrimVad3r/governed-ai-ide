...
import { ActiveIntent } from "../hooks/types";

/**
 * Returns the governance block to prepend to every system prompt.
 * This text enforces the Plan-First / Intent-Driven protocol.
 */
export function getGovernanceSystemPrompt(params?: {
  agentLabel?: string;
  currentIntents?: ActiveIntent[];
}): string {
  const agentLabel = params?.agentLabel ?? "Builder";
  const intentsBlock =
    params?.currentIntents && params.currentIntents.length > 0
 ...
 */
export function getTurnReminderPrompt(currentIntentId: string | null): string {
  if (!currentIntentId) {
